...
#         logger.error(f"URL validation failed: '{url_str}'")
#         raise ValueError("Invalid URL, make sure the URL is a non-empty string")
    
#     # Basic URL format check
#     parsed_url = urlparse(url_str)
#     if not parsed_url.scheme or not parsed_url.netloc:
#         logger.error(f"Invalid URL format: '{url_str}' (scheme: {parsed_url.scheme}, netloc: {parsed_url.netloc})")
#         raise ValueError("Invalid URL, must include scheme (e.g., https) and domain")

#     try:
#         async with AsyncWebCrawler() as crawler:
#             logger.info(f"Starting crawl for {url_str}")
#             result = await crawler.arun(url=url_str)
#             if not result.success:
#                 logger.error(f"Crawl failed for {url_str}: {result.error_message}")
#                 raise ValueError(f"Failed to crawl the website: {result.error_message}")
#             knowledge_base[url_str] = result.markdown
#             logger.info(f"Successfully crawled and stored content for {url_str}")
#     except Exception as e:
#         logger.error(f"Crawling failed for {url_str}: {type(e).__name__}: {str(e)}")
#         raise
from crawl4ai import AsyncWebCrawler
import logging
from typing import Dict
